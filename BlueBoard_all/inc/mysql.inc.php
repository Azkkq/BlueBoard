...
function execute_multi($link,$arr_sqls,&$error){
	...
	if(mysqli_multi_query($link,$sqls)){
		...
		if($i==count($arr_sqls)){
			...
			$error="sql statement executed failed：<br />&nbsp;line:{$i}的语句:{$arr_sqls[$i]} error:<br />&nbsp; reason：".mysqli_error($link);
			return false;
		}
	}else{
		$error='Executed failed! Please check first statement!<br /> Probable cause:'.mysqli_error($link);
		return false;
	}
}
//获取记录数
